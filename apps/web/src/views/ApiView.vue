...
// ---- Tokens state（合并自 ApiTokensView） ----
...
const activeToc = ref<string>('tokens');
...
onMounted(() => {
  void nextTick(() => {
    ...
    // 若是 /me/api-tokens 重定向过来（?to=tokens），自动滚动到 tokens
    const to = route.query.to;
    if (typeof to === 'string' && sections.some((s) => s.id === to)) {
      jumpTo(to);
    }
  });
});
        ...
        <article class="docs">
          ...
          <section id="tokens">
            <div class="section-head">
              <h2>我的 Token <span class="han">· Bearer Credentials</span></h2>
