...
/**
 * 扬力品牌错误页通用骨架 — "打印失败"叙事（VOID 印章版叠纸 + 编辑级文案）。
 * 404 / 403 / 500 共用同一套版面，仅数字 / 印章字 / 文案 / HTTP code 不同。
 */
import { ref, onMounted } from 'vue';
import { useRouter } from 'vue-router';

interface Props {
  numLeft: string; // 红方块左侧数字，如 '4'
  numRight: string; // 红方块右侧数字，如 '4'
  stamp: string; // 印章文字：VOID / DENIED / ERROR
  eyebrow: string; // mono eyebrow：'Misprint · 寻址失败'
  titleCn: string;
  titleEn: string;
    ...
    <main class="center">
      ...
      <div class="geom" aria-hidden="true">
        ...
        <div class="doc front">
          ...
          <div class="ln gray short"></div>
          <div class="ln fail long"></div>
          <div class="void-stamp">{{ stamp }}</div>
        </div>
      ...
      <section class="copy">
        ...
        <h1 class="num">{{ numLeft }}<span class="accent"></span>{{ numRight }}</h1>

        <h2 class="title-cn">{{ titleCn }}</h2>
        <div class="title-en">{{ titleEn }}</div>

        <p class="body">
          <template v-for="(line, i) in bodyLines" :key="i">
            {{ line }}<br v-if="i < bodyLines.length - 1" />
          </template>
        </p>
