...
**最近更新**：2026-05-28（fix(docker)：api 生产镜像改用 `pnpm deploy` 产出自包含 node_modules，修运行时 `Cannot find module`(GAP#1)+ undici 误标 devDep + Prisma OpenSSL3 引擎；镜像现可 build 且能启动至 `validateEnv`）
