...
**最近更新**：2026-05-28（fix(deploy)：迁移改在 api 起服务前用 `docker compose run --rm` 执行 + `EmergencyAdminBootstrap` 容忍空库(P2021/P2022)，修空库首次部署 api 崩溃循环导致一把跑通失败(GAP#3)；连带 init/update.sh source `.env.prod` 修镜像 tag 插值(GAP#2)）
