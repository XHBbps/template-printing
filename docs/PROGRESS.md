...
- **fix(renderer)：文字元素对齐左右对称(通用,全模板生效)** —— `TextElement`(设计器画布/打印/预览复用同一组件)把 letter-spacing 与下划线从容器下沉到内层文字 run:① center/right 时加 `margin-right: -letterSpacing` 抵消"最后一个字后的尾部字距",使居中真正左右等距(原先整块文字被尾距往左推);② `textAlign='justify'` 改用**真·分散对齐**(`text-align: justify; text-align-last: justify` 块级)—— CJK 字符在框内等距铺满、首尾贴边、左右完全对称,下划线随之边到边对称(原先 justify 被映射成无效的 flex space-between);③ 有下划线时加 `text-underline-offset: 0.15em` 与字体留间距。不改任何模板数据。
