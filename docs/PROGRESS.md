...
- **飞书客户端「免登」(收窄版):飞书 webview 内 + 无会话自动走 Lark OAuth 直进工作区**(纯前端,OAuth 回调后端不动)—— 灯塔需求:在飞书客户端内打开应用的内部用户不应再手动点登录。**触发**:仅当 UA 含 `Lark`/`Feishu` 且无会话时,在「无会话 → 落 `/login`」的执行点——`router/index.ts` `beforeEach`(子序导航 + 无会话直达 `/login`)与 `AppShell.vue` `enforceAfterHydrate`(首屏 hydrate 落定仍无会话)——整页跳现有 `/api/auth/lark/login`(经 `maybeStartLarkAutoLogin`);外网浏览器 / 被退出抑制 / 已尝试过则维持「手动登录页」现状,**登录页组件零改动**。**新增** `apps/web/src/lib/lark-auto-login.ts`:`isFeishuClient()`(UA 判定)、`maybeStartLarkAutoLogin(continueTo)`(命中即 `window.location.assign` 跳转、返回 true 让调用方取消落 `/login`)、`markLarkLogout()` / `clearLarkAutoLoginMarkers()`。**两 `sessionStorage` 标记保安全**:① 退出抑制(`tp_lark_logout`)——`stores/auth.ts` `logout()` 在 `user` 置空**前**同步设,本会话内不再自动登入,否则登出即被秒登回(spec 🔴);② 防循环(`tp_lark_auto_tried`)——发起自动跳转**前**置位,若回跳后仍无会话(又走到判定点且见标记)= 本轮 OAuth 失败,放弃自动跳、落登录页(spec 🔴),`emergency_admin` 账密入口天然可用。两标记在登录成功(`hydrate` / `tryRefresh` 拿到 `user`)时清除,恢复后续自动登入。`sessionStorage` 不可用(隐私模式 / 部分 webview)优雅降级为不自动登入。**验证**:`vue-tsc` typecheck + `eslint` + `vite build`(3359 模块)全过。**⚠ 真机实测暂缓(gated on HTTPS)**:灯塔生产实测撞 HTTP 下飞书 webview 的 cookie 限制——webview 嵌入(第三方)上下文要 cookie 跟随必须 `SameSite=None+Secure`(需 HTTPS),HTTP 内网部署只能 `Lax` → webview 内会话约 2–3 分钟失活 → 免登检测无会话即自动重登 → 又失活 → **死循环**(免登成功会清防循环标记,故标记拦不住「成功→快速失活→再登入」这类循环;OAuth/JWT 24h/cookie 持久均非因)。免登前端门控逻辑本身经外网浏览器验证无误,纯受限于 HTTP cookie。**决策**:web 镜像回退 v1、生产暂缓,`cad7eccb` 留 master、`web:v2` 镜像留 Harbor 备用。**待 HTTPS 就绪**:`env.secure` 由 `LARK_SSO_REDIRECT_URI` 协议自动切 `None+Secure`(见 2026-06-11 cookie 自适应),前端代码零改动,出 web 新镜像即可启用;启用时复测「会话过期→自动重登」路径确认根因已消。
- **api entrypoint 启动期修共享 `/storage` 卷归属,解 render 非 root 写 EACCES**(commit `b9d01777`)—— 灯塔生产渲染实战暴露真 bug:Pharos 共享命名卷默认 `root:root 755`,render 容器跑非 root(`uid 100:101`)→ `mkdir`/写 `/storage/uploads` EACCES;api 跑 root 故其写入(图片/附件缓存)一直正常、**掩盖此 bug**,render 首次真写渲染产物才暴露(历次验收只跑过 api 写)。**修法(方案 A)**:新增 `apps/api/docker-entrypoint.sh`,api(root,且 render `depends_on api:service_healthy` 先起)启动 node 前 `mkdir -p /storage/uploads/render` + `chown` 卷归属给 `100:101` —— 全新卷自愈,与 Prisma migrate 自动化同思路;**优化**:仅顶层 owner≠100 时才 `chown -R`(首启一次性),已修卷走非递归,避免每次启动对积累产物全量 chown 拖慢起动/卡健康门。`Dockerfile.prod` runtime stage COPY 脚本(在构建上下文,非 `/prod/api`)+ `sed` 去 CRLF + chmod +x,ENTRYPOINT 串 tini 后 exec 原 CMD。**验证**:本地现有镜像 + fresh 卷实证——未修 render 直写 EACCES 复现 → entrypoint chown 后 render 写 RENDER_OK,ENTRYPOINT 链日志确认启动期执行,幂等第二次跳过 `-R`。出 v3 镜像由灯塔构建机重推。

### 2026-06-11

- **SSO/会话 cookie 按部署形态自适应,修 http+反代下回调 400 与登录态拒存**(commit `9e3726b4`)—— 灯塔 Pharos 部署实战实测根因(双因):① lark state cookie `Path` 写死容器内部路径 `/auth/lark/callback`,经 web 反代后浏览器真实路径带 `/api` 前缀 → path 不匹配 cookie 不发送 → callback 400 `Invalid OAuth state`;② `NODE_ENV=production` 即 `SameSite=None+Secure`,`http://内网ip` 下浏览器拒存(None 强制 Secure 死锁)。**会话 cookie**(`jwt-cookie.helper.ts`)同因② —— state 修好后登录态仍存不住,一并修。**修法**:`CookieEnv.nodeEnv` → `secure`(由 `LARK_SSO_REDIRECT_URI` 协议判定,`auth.module.ts` 单点计算):http → 省略 Secure + `SameSite=Lax`(OAuth 回调是顶级导航 GET,Lax 放行);https → 保持 `None+Secure`(飞书 webview 需要,上证书后自动回正)。state cookie `Path` 改 `'/'`。**验证**:auth/cookie 相关 11 套件 45 测试全绿 + typecheck。⚠ 本地 dev 容器跑 auth e2e 须显式 `LARK_SSO_REDIRECT_URI=http://localhost:3000/...`(容器 env_file 注入的隧道 https URI 优先于 `.env.test` → `secure=true` → supertest 拒回传 Secure cookie 而误挂;CI 在 `ci.yml` 显式 http,不受影响)。
- **灯塔 Pharos 平台适配落库**(commits `7ffdd5a2`/`dadf8887`/`adfdd5e7`/`71921337`/`e3a5a248`)—— 详见 `handoff/pharos-adaptation.md` + `pharos-env-keys.md`:web 镜像内置 `/api`+`/uploads` 反代(`docker/web-nginx.conf`,storage 全经 api 出、web 不挂卷);新增 `docker-compose.pharos.yml`(平台形态:相对镜像路径/无 ports/env_file/命名卷,与 GHCR prod 形态并存);api 镜像构建期 `PRISMA_CLI_BINARY_TARGETS=linux-musl-openssl-3.0.x` 预取 schema 引擎(免内网部署期 `migrate deploy` 去 binaries.prisma.sh 卡死);`.dockerignore` 排除 `.pnpm-store`/`storage`。§4.1–4.4 本地验收通过,5 镜像 Harbor 已推送(灯塔侧)。

### 2026-06-06

- **用户管理操作列对齐统一**(commit `e772f2bb`)—— 操作列三控件原本对齐不一致(只读 span 用 flex 居中,而 `.act-btn` 按钮 / `.role-select` 下拉都没居中)。`.act-btn`(重置密码/禁用/启用)加 `inline-flex` 居中 + `line-height:1` → 文字垂直+水平稳定居中;`.role-select`(角色下拉)`appearance:none` 去原生外观 + 自定义箭头,文字**左对齐**、箭头右置(原生 `<select>` 按最宽选项「普通用户」定宽,短值「管理员」居中+右箭头会左侧空洞,故用标准左对齐下拉样式)。纯 `UsersAdminView.vue` scoped CSS,`vite build` 通过。

### 2026-05-30

- **新建模板加重入守卫,防连点产生多份**(commit `f667c9f4`)—— `TemplatesView.vue` `createNew()` 在 `await templates.create()`(`POST /templates`,后端无幂等)期间**无重入防护**、新建卡不禁用,连点 N 次建 N 份「未命名模板」(经隧道 await ~1s 窗口更大易触发)。修复:加 `creating` ref 守卫(`if (creating.value) return` + `finally` 复位),两处新建卡绑 `:class="{ 'is-busy': creating }"` → 进行中 `pointer-events:none` 禁点 + 降透明给反馈。`vite build` + `vue-tsc` 通过。(「新建空白模板」无天然去重键,前端重入守卫为此类重复提交的标准修法;后端 idempotency-key 纵深防御留待按需。)
- **飞书机器人模板选择:本人私有已发布可见 + 分页**(commit `49bde6ea`)—— 机器人选择卡片的可见范围从「仅 public+published」扩为「已发布 且 (公开 OR `ownerId`=本人)」:发起人飞书 `open_id` ↔ 平台账号 `larkOpenId` 解析(经 SSO 登录过才有绑定;无匹配则仅见公开),`botTemplateWhere(userId)` 替代原 `BOT_TEMPLATE_WHERE` 常量。**防越权保留**——他人私有模板永不可见/不可渲染,校验在「列表 + `template_selected` + `submit_render`」三处一致(`apps/api/src/lark/lark-bot-dispatch.service.ts`)。**分页(无需 DB schema 改动)**:`listBotTemplates({userId,page,pageSize})` 返回 `{templates,total,page,pageSize}`,卡片 `buildSelectTemplateCard`(`lark-bot-cards.ts`)加「第 X/Y 页 · 共 N 个」+ 上/下页按钮,页码编进按钮 `value.page`、回调 `action=select_page` 换卡片(普通按钮非 form,飞书原样回传 value),`lark-bot-payload.ts` 的 value 类型加 `page?`;每页 20。**验证**:`lark-bot-*` 全部测试 **35/35**(含 owner 私有可见 / 跨用户隔离 / 分页断言)、api typecheck 通过。
- **侧边栏头像显示飞书头像,加载失败回退首字母**(commit `6a76795b`)—— `AppSidebar.vue` 底部头像原写死渲染姓名首字母、从不引用 `avatarUrl`;改为有 `avatarUrl` 即渲染 `<img>`(`referrerpolicy=no-referrer` + `@error` 置 `avatarFailed` 回退),缺失/加载失败才显示首字母。`app-shell.css` 加 `.avatar{overflow:hidden}` + `.avatar img{object-fit:cover;border-radius:inherit}` 让头像撑满圆角裁切。(后端早已正确存 `avatar_url` 并由 `/users/me` 返回,顶栏头像本就正常,仅侧栏渲染缺口。)
- **飞书登录点击即反馈 + 预登出改非阻塞**(commit `9a772ef2`)—— `LoginView.vue` `goLark` 原先 `await authStore.logout()`(一次慢往返)再 `window.location.assign` 整页跳转,全程无加载提示、点击后约 3s「无反应」。改为:点击立刻 `redirecting=true`(按钮禁用 + 文案「正在跳转飞书…」,防重复点击),预登出改 `void authStore.logout().catch()` 非阻塞——即将跳转飞书、SSO 回调本会签发新 cookie 覆盖旧会话,该预登出冗余,去掉其阻塞往返。

### 2026-05-29

- **飞书 bot 长连接(WebSocket)接管事件 + 卡片回调** —— 飞书后台已切长连接模式,HTTP 事件订阅收不到事件。依 `docs/superpowers/specs/2026-05-29-lark-bot-longconn-design.md` + plan。**架构**:抽 `LarkBotDispatchService`(纯业务,收归一化 payload)+ `LarkBotWsService`(薄 WS 胶水)。**关键**:SDK `@larksuiteoapi/node-sdk@1.40.0`(已是依赖)`EventDispatcher.parse()` 把 v2 事件的 `header`+`event` 扁平化到顶层 → handler 收到的 `targetData` 字段路径与 HTTP body 不同,故新增 `lark-bot-payload.ts` 归一化适配(`fromWs*`/`fromHttp*`),WS 与 HTTP 两入口各自适配后调同一 dispatch。`LarkBotWsService` 受 `LARK_BOT_LONG_CONN_ENABLED`(默认 false/生产单副本 true)门控,`new WSClient({appId,appSecret,domain:Domain.Feishu,autoReconnect})` + 注册 `im.message.receive_v1`/`card.action.trigger`;启动失败只 warn 不阻塞;`OnApplicationShutdown`(配套 `app.enableShutdownHooks()`)。**HTTP 端点保留作 fallback**(瘦身为适配壳调 dispatch,删除留待 WS 生产稳跑后 cleanup)。`extractFields` 移入 `lark-bot-cards.ts`。env 加 `LARK_BOT_LONG_CONN_ENABLED`,批次5 的 bot 断言改为以"长连接/token 启用"触发、只看 `LARK_BOT_OPEN_ID`。**验证**:新增 `lark-bot-payload.spec`(2)/`lark-bot-dispatch.spec`(5:私聊/群@/自回环/去重)/`lark-bot-ws-gating.spec`(3);现有 bot e2e 经适配仍走 fallback;typecheck 通过。**待**:真连接 + 卡片往返手测(需飞书侧 @/点卡片,go/no-go 闸,过后方删 fallback)。
- **运行时告警:渲染卡死 / 回调耗尽 → 飞书运维群(@所有人)** —— 部署通知(CI 侧)之外补运行时健康告警(后端侧)。`LarkImService` 加 `sendTextToChat(chatId, text)`(镜像 `sendTextToUser`,`receive_id_type=chat_id`,文本含 `<at user_id="all"></at>`=@所有人)。`RenderCleanupService` 注入 `LarkImService`(@Global LarkModule 导出,无需 import 避免与其循环依赖),加 `sendOpsAlert`(读 `LARK_ALERT_CHAT_ID`,空=关 fail-safe;fire-and-forget,告警失败不影响业务):**① stuck**——`reconcileStuckJobs` 翻转 stuck job 后推"N 个任务超时";**② 回调耗尽**——`resendFailedCallbacks` 中 `resendOne` 改返 ok,本轮重试达上限仍失败的 job 聚合后推 jobIds。借对账 cron 5min 周期天然聚合不刷屏。env `LARK_ALERT_CHAT_ID` 服务直接读 `process.env`,入 `.env.prod.example` + `env-example-sync.spec` 白名单;同步 `PRE_DEPLOYMENT_CHECKLIST`(运维群段拆部署通知/运行时告警两类)。**验证**:`render-stuck-reconcile.e2e` 加告警用例(翻转 stuck → 调 sendTextToChat 含目标群+@所有人+stuck_timeout)、`callback-resend.e2e` 适配 4 参构造;两文件 + env-sync/exception/orphan 各文件单跑全绿,typecheck 通过(全量套件单进程跑因开发机物理内存吃紧 OOM,与本改动无关——告警为 opt-in 且 env 在测试未设)。
- **deploy.yml 接入飞书部署告警(填补 TODO)** —— `deploy.yml` 原成功/失败通知是 `# TODO`。改为部署成功/失败时 `curl` POST 文案到飞书自定义机器人(text 消息,含版本/仓库/触发人/run 链接)。webhook 走可选 GitHub Secret `LARK_DEPLOY_WEBHOOK`,job 级 env 暴露后用 `if: success()/failure() && env.LARK_DEPLOY_WEBHOOK != ''` 守卫——未配则跳过、不影响部署。机器人安全建议用「自定义关键词」+ 关键词「部署」(文案含该词);加签需另补签名逻辑。同步更新 `PRE_DEPLOYMENT_CHECKLIST.md`(GitHub Secrets 加 `LARK_DEPLOY_WEBHOOK`、运维群段从"待实现"改为已接入)。YAML 校验通过。
- **PRE_DEPLOYMENT_CHECKLIST 对齐实际部署路径(GHCR + 服务器 .env.prod)** —— 清单写于早期 ACR 方案,与现状(release→GHCR `tp-api/tp-web/tp-render`、`deploy.yml` SSH→`update.sh` 载入服务器 `.env.prod`)严重漂移,照旧执行会踩坑。按 AGENTS §9.3「代码 vs 文档冲突→修文档」一次性修正:① ACR 段重写为 GHCR(GITHUB_TOKEN 自动推、私有镜像服务器需 `docker login ghcr.io`+`read:packages` PAT);② GitHub Secrets 仅留 `DEPLOY_HOST/DEPLOY_USER/DEPLOY_SSH_KEY`(原 `ECS_*`/`ALIYUN_ACR_*` 名错),应用密钥归位服务器 `.env.prod`(指向权威 `.env.prod.example`);③ 飞书 redirect URI 补 `/api` 前缀(`/api/auth/lark/callback`,漏则 SSO 回调 404);④ 补现需 env(`RENDER_CALLBACK_SECRET`〔批次5 生产启动断言〕/`LARK_BITABLE|BOT_VERIFICATION_TOKEN`/`LARK_BOT_OPEN_ID`/`CORS_ORIGIN`/`COOKIE_DOMAIN`);⑤ 告警 webhook 标注为 deploy.yml `# TODO` 未实现。纯文档,无代码改动。
- **bug-review 复核收尾:A2 测试 DB 端口对齐 dev 栈(本地直跑开箱可连)** —— 复核指出 `.env.test`(5432/6379)+ render `db.ts` 默认 host `postgres` 与 `docker-compose.dev.yml` 宿主机映射(6432/6479,避让 Windows 保留端口段)不符,本地裸机直跑 `pnpm test` 连不上。查清:CI(`ci.yml`)在 job env **显式设** 5432/6379,dotenv/`??` 守卫不覆盖已存在 env → 改本地回退值对 CI 零风险。**修复**:`apps/api/.env.test` 的 `DATABASE_URL`/`REDIS_URL` 改 6432/6479;`apps/render` 加 `vitest.config.ts` + `test/setup.ts`(仅当 `DATABASE_URL` 未设时回退 localhost:6432)。三环境各自正确:CI(job env 5432)/ 容器内(compose `postgres:5432`)先设好 → 守卫不覆盖;本地裸机未设 → 用回退。**验证**:不带任何 env 覆盖直跑,api DB 用例 + render 17/17 全绿。
- **bug-review 复核收尾:A1 重置密码吊销 token(P2 安全)+ A3 闸门 e2e 上 filter 提保真** —— 复核 agent 指出两处。**A1**:`resetPassword`(`users.controller.ts`)此前只 evict 缓存、不吊销 token,而 `disable` 调了 `revokeAllForUser`(refresh)+`apiTokens.revokeAllForUser`。后果:admin 重置失陷账号密码后,攻击者旧 refresh/API token 未吊销,强制改密闸仅在 `mustChangePassword=true` 期间临时挡住,一旦合法用户改密(标志转 false)旧 token 立即恢复全权——重置无法用于失陷处置。**修复**:`resetPassword` 补 `refresh.revokeAllForUser` + `apiTokens.revokeAllForUser`(try 内吊销、finally evict,与 disable 完全对齐)。**A3**:`must-change-password-gate.e2e` 此前断言 Nest 原始 `body.code`(e2e 未装 `GlobalExceptionFilter`),与生产经 filter 包装的 `body.error.code` 不符 → 捕捉不到 filter 层 bug;改为 e2e 装上 `GlobalExceptionFilter` 并断言 `body.error.code`,与生产一致。**验证**:`users-reset-password.e2e` 加吊销断言(重置后该用户 refresh+API token 均 revokedAt 非空);全量 api **208 passed** + typecheck 通过。
- **bug-review 第 7 节「需进一步验证项」三项竞态:集成测试复现 + 定论** —— 用集成测试逐项确认是否真实存在再决定修否(api 用 jest,与项目一致)。**① refresh 并发分叉 → 确认真实、已修**:`verify`(读 revokedAt=null)+`revoke`+`create` 非原子,同一 token 并发两次 refresh 时两请求都能 verify 通过 → 各自 create → 分叉两套有效会话(service 层确定性复现:两次 `verify` 都非 null)。**修复**:`RefreshTokenService` 加 `revokeIfActive(id)`(`updateMany({where:{id,revokedAt:null}})` CAS,返回 count===1),`auth.controller` refresh 改为仅赢得 CAS 者继续签发、输家 401(『Refresh token already used』,兼最小 token 重用检测)。`refresh-concurrency.e2e` 断言并发后用户名下有效 token 仅 1 个 + service 层 CAS 只赢一次。**② stuck cron 误判 → 确认非双回调 bug,不改**:批次1 终态守卫(render `markProcessing/markDone` 带 `status NOT IN(done,failed)`)+ reconcile 的 `status='processing'` 守卫,使 cron 先翻 failed 后慢 worker 的 markDone 影响 0 行(终态粘性)、且 worker 仅 rowCount>0 才发 done 回调 → 全程仅 cron 1 次 stuck 回调,无双回调/双计数(`render-stuck-reconcile.e2e` 加『cron 先翻、慢 worker markDone 影响 0 行、只 1 次回调』用例)。代价:确成功但超 10min 的慢 job 会判 failed —— 超时设计取舍,非 bug。**③ 孤儿清理正则 → 确认不漏算,不改**:上传图片恒存为 `/uploads/<sha256>.<ext>`(`uploads.service.storeImage` 返回、设计器原样存),正则 `/\/uploads\/([A-Za-z0-9._-]+)/g` **非锚定**——绝对 URL `https://cdn/uploads/<file>` 命中子串、带 query `/uploads/<file>?v=2` 在 `?` 处停止取纯文件名,均不漏算(`orphan-uploads-cleanup.e2e` 加两形态旧文件『被引用即存活』用例)。唯一理论遗漏(仅经运行时 render 数据引用、从不入 template/version data 的上传)非正常设计流,且有内容寻址 + 7 天宽限兜底。**验证**:全量 api **207 passed** + typecheck 通过。
- **bug-review 批次5:P2 共性健壮性(safeParse + filter details + listJobs 校验 + env 启动断言)** —— 依 review 第 1/3/5 节。**① `.parse()`→`safeParse`**:`me.controller`(updateProfile/setPassword)、`local.controller`(login)三处由 `.parse()` 抛 `ZodError`(非 HttpException)→ 过滤器判 500 + 误报 Sentry,改 `safeParse` + `throw new BadRequestException(parsed.error.flatten())`,与其余 controller 一致(非法 body 返 400)。**② 过滤器填充 details**(`common/exception.filter.ts`):识别 object 形态的 `getResponse()`——无字符串 `message`(典型 zod flatten `{formErrors,fieldErrors}`)时把完整内容放进 `error.details`(前端拿字段级错误,不再只见笼统 "Bad Request");有 `code` 字段(如 `MUST_CHANGE_PASSWORD`)透传到 `error.code`。**③ listJobs 分页校验**(`render/render.controller.ts`):`page/pageSize` 改 zod `z.coerce.number().int().min(1)`(pageSize `.max(100)`)+ 非法返 400,消除 `?page=abc`→`NaN`→Prisma 500(全仓最后一个漏网列表端点)。**④ env 漏配启动断言**(`common/env.ts` `validateEnv`):配了 `LARK_BITABLE_VERIFICATION_TOKEN` 却缺 `RENDER_CALLBACK_SECRET`(回调永久 401 卡处理中)→ **生产硬阻断启动 / 非生产 warn**;配了 `LARK_BOT_VERIFICATION_TOKEN` 却缺 `LARK_BOT_OPEN_ID`(群消息被静默吞)→ warn。**验证**:新增 `exception-filter.spec`(flatten→details / 自定义 code 透传 / 字符串 message / 500 不泄露)、`render-listjobs-validation.e2e`(page=abc/pageSize=0 → 400、合法 → 200)、`env.spec` 补 3 例(生产阻断 / 非生产 warn / bot warn);全量 api **203 passed** + typecheck 通过,无回归。
