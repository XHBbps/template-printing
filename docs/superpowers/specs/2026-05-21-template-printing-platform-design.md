...
- GitHub Secrets 存放：`ALIYUN_ACR_USERNAME` / `ALIYUN_ACR_PASSWORD` / `ECS_SSH_PRIVATE_KEY` / `ECS_HOST` / `JWT_SECRET` / `FILE_SIG_SECRET` / `DATABASE_URL` / `REDIS_URL` / `LARK_SSO_APP_ID` / `LARK_SSO_APP_SECRET` / `LARK_SSO_REDIRECT_URI` / `INITIAL_ADMIN_LARK_USER_IDS` / `INITIAL_ADMIN_LOCAL_PASSWORD`。（`MASTER_KEY` 已移除 — 见 D-A2）
